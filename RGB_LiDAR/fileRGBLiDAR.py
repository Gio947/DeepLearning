...
# Array che contiene tutti i file del training set
name_training = ['train-00000-of-00021-e8c1d8ec39d4e7f8', 'train-00001-of-00021-0fe57e09489e73fb',
...
# Array che contiene tutti i file del test set
name_testing = ['task1_test-00000-of-00004-90b9f1aa82f72da4', 'task1_test-00001-of-00004-20f6d3e56f215d4a']
# Array che contiene tutti i file del validation set
name_validation = ['validation-00000-of-00005-aee5d045c6504154']

# Realizzazione dei subset per il training, validation e testing
subset = Subset(name_training, name_testing, name_validation, bs_train, bs_test, bs_final_test, dim_img)
...
writer = SummaryWriter('./runs/provaRGB_LiDAR')

PATH = './modello/unet_rgb_lidar2.pth'
...
def train(model, epochs):
    ...
    for epoch in range(epochs):
        ...
        for batch, (lidar, images, targets) in loop:
            ...
            with torch.no_grad():  # Evita di calcolare i gradienti durante la valutazione
                if batch == 1:
                    ...
                    (lidar, img, mask) = next(iter(test_dataloader))
                    lidar = lidar.to(device)
                    img = img.to(device)
                    mask = mask.to(device)
                    mask = mask[0]
                    pred = model(lidar,img).detach()
                    ...
                    if lidar_np.ndim == 4:  # Se l'immagine ha un batch dimensionale
                        lidar_np = lidar_np[0]  # Prendi il primo elemento del batch
                    lidar_grid = np.transpose(lidar_np, (1, 2, 0))  # Trasponi da (C, H, W) a (H, W, C)
                    ...
                    writer.add_image('Original Image LiDAR', lidar_grid, global_step=batch, dataformats='HWC')
                    writer.add_image('Original Image RGB', img_grid, global_step=batch, dataformats='HWC')
        ...
        with torch.no_grad():
            for test_batch, (test_lidar, test_images, test_targets) in enumerate(test_dataloader):
                test_lidar = test_lidar.to(device)
                test_images = test_images.to(device)
                test_targets = test_targets.to(device)
                test_pred = model(test_lidar.detach(), test_images.detach())
...
def test(model, final_test_dataloader):
    ...
    for i, (lidar, img, mask) in enumerate(islice(final_test_dataloader, 3)):
        lidar = lidar.to(device)
        img = img.to(device)
        mask = mask.to(device)
        pred = model(lidar, img)
        
        # Original Image RGB
        writer.add_image(f'Test Original Image RGB {i+1}', np.transpose(img[0].cpu().numpy(), (1, 2, 0)), global_step=i, dataformats='HWC')
        
        # LiDAR Image
        writer.add_image(f'Test Original LiDAR {i+1}', lidar[0].cpu().numpy().squeeze(0), global_step=i, dataformats='HW')
        ...
        # Visualizza le immagini, la maschera e la previsione
        plt.figure(figsize=(12, 12))
        plt.subplot(1, 4, 1)
        plt.imshow(np.squeeze(img[0].cpu().numpy()).transpose(1, 2, 0))
        plt.title('Original Image')
        plt.axis('off')
        plt.subplot(1, 4, 2)
        plt.imshow(np.squeeze(lidar[0].cpu().numpy()))
    ...
    # Chiudi il writer una volta finito il testing
    writer.close()
