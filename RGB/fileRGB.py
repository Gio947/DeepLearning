...
# Array che contengono tutti i file del dataset
...
writer = SummaryWriter('./runs/provaRGB')

#comando TensorBoard ---> tensorboard --logdir=runs

# Nome e percorso su cui salvare il modello
PATH = './modello/unet_rgb7.pth'
...
# Metodo che esegue il test del modello
def test(model, final_test_dataloader):
    ...
    # Itera su 3 le immagini nel dataloader
    for i, (img, mask) in enumerate(islice(final_test_dataloader,3)):
        ...
        writer.add_image(f'Test Prediction', pred_grid, global_step=i, dataformats='HW')
        
        # Visualizza l'immagine, la maschera e la previsione
