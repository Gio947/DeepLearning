...
class Subset(object):
    def __init__(self, name_training, name_testing, name_validation, bs_train=32, bs_test=16, bs_final_test=16, dim_img=256):
        ...
        # Datasets
        self.train_dataset = MRIDataset(self.df_training['image'], self.df_training['mask'], self.dim_img)
        self.test_dataset = MRIDataset(self.df_validation['image'], self.df_validation['mask'], self.dim_img)
        self.final_test_dataset = MRIDataset(self.df_testing['image'], self.df_testing['mask'], self.dim_img)
